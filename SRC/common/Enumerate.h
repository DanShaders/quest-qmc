...
class Enumerator {
    using IteratorType = decltype(std::declval<Iterable>().begin());
    using ValueType = decltype(*std::declval<IteratorType>());
...
public:
    Enumerator(Iterable&& iterable)
        : m_iterable(std::forward<Iterable>(iterable))
...
{
    return detail::Enumerator<T> { std::forward<T>(range) };
}
